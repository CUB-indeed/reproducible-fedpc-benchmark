...
from scipy.stats import pearsonr, norm, wilcoxon, t as t_dist, spearmanr
import random
import time
import os
import pandas as pd
...
def communication_cost(client_edges, p, ell):
    """
    Communication cost approximation, in bytes.

    Each client transmits (i) a binary skeleton indicator over all C(p,2)
    candidate variable pairs, and (ii) for every retained edge, its Sepset
    encoded as variable indices. Each variable index requires
    ceil(log2(p)) bits and each skeleton indicator requires 1 bit, so the
    payload for client k is:

        Comm_k = C(p,2)/8 + |E_k| * ell_max * ceil(log2(p))/8   bytes

    and the total communication cost is the sum of Comm_k over all clients.
    """
    n_pairs = p * (p - 1) / 2
    bits_per_index = np.ceil(np.log2(p)) if p > 1 else 1.0
    total_bytes = 0.0
    for edges in client_edges:
        comm_k = n_pairs / 8.0 + len(edges) * ell * bits_per_index / 8.0
        total_bytes += comm_k
    return total_bytes
...
def run_single_experiment(
    ...
    results["local"] = {
        ...
        "Comm": communication_cost(client_edges, p, ell), "Runtime": time.time() - t0
    }
...
def generate_all_figures(df_main, df_stat, out_dir="results/figures"):
    ...
    ax.set_ylabel("Communication cost (bytes)", fontsize=9)
    ...
    ax.set_xlabel("Communication cost (bytes)", fontsize=10)
    ax.set_ylabel("Skeleton F1", fontsize=10)
    ax.legend(fontsize=9)
    plt.tight_layout()
...
REPS_DEFAULT = 20
OUT_DIR = "results/next_section"

# ======================================================================
# SECTION 1 — Factorial ablation
# ======================================================================

FACTORIAL_CONDITIONS = {
    "equalW_noSep":            dict(skel="equal",     sep=None),
    "consensusW_noSep":        dict(skel="consensus", sep=None),
    "equalW_equalSep":         dict(skel="equal",     sep="equal"),
    "consensusW_equalSep":     dict(skel="consensus", sep="equal"),
    "consensusW_consensusSep": dict(skel="consensus", sep="consensus"),
}


def _uniform_weights(K):
    return np.ones(K) / K


def _run_factorial_condition(client_edges, client_seps, true_skel, true_dir,
                              p, w_con, tau, cond):
    spec = FACTORIAL_CONDITIONS[cond]
    K = len(client_edges)
    w_skel = _uniform_weights(K) if spec["skel"] == "equal" else w_con

    pred_skel = weighted_aggregation_with_weights(client_edges, w_skel, tau)

    if spec["sep"] is None:
        agg_seps = {}
    elif spec["sep"] == "equal":
        agg_seps = aggregate_sepsets(client_seps, _uniform_weights(K), tau)
    else:
        agg_seps = aggregate_sepsets(client_seps, w_con, tau)

    G = nx.Graph()
    G.add_nodes_from(range(p))
    G.add_edges_from(pred_skel)
    cpdag = orient_v_structures(G, agg_seps)

    shd, prec, rec, f1 = compute_metrics(true_skel, pred_skel)
    n_or, dp, dr, df1 = orientation_metrics_cpdag(true_dir, cpdag)
    return {"SHD": shd, "F1": f1, "Precision": prec, "Recall": rec,
            "n_oriented": n_or, "Dir_Prec": dp, "Dir_Rec": dr, "Dir_F1": df1}


def _factorial_single(p=20, N=5000, K=5, edge_prob=0.2, hetero="mild",
                       alpha=ALPHA, tau=TAU_DEFAULT, ell=ELL_MAX):
    B = generate_random_dag(p, edge_prob)
    true_skel = true_skeleton_edges(B)
    true_dir = true_directed_edges(B)
    ...
    for cond in FACTORIAL_CONDITIONS:
        out[cond] = _run_factorial_condition(
            client_edges, client_seps, true_skel, true_dir, p, w_con, tau, cond
        )
    return out


def run_factorial_ablation(reps=REPS_DEFAULT, **kwargs):
    print(f"[1/4] Factorial ablation ({reps} reps) ...")
    rep_results = [_factorial_single(**kwargs) for _ in range(reps)]

    rows = []
    for cond in FACTORIAL_CONDITIONS:
        row = {"condition": cond}
        for k in ["SHD", "F1", "Precision", "Recall", "Dir_F1", "Dir_Prec", "Dir_Rec", "n_oriented"]:
            vals = [r[cond][k] for r in rep_results]
            row[f"{k}_mean"] = float(np.mean(vals))
            row[f"{k}_std"] = float(np.std(vals))
        rows.append(row)
    df_summary = pd.DataFrame(rows)

    # Pairwise Wilcoxon tests isolating each factor's marginal effect.
    comparisons = [
        ("consensusW_noSep", "equalW_noSep",
         "Skeleton-weighting effect alone (Sepset aggregation off both sides)"),
        ("consensusW_equalSep", "consensusW_noSep",
         "Sepset-aggregation effect (skeleton weight fixed = consensus)"),
        ("consensusW_consensusSep", "consensusW_equalSep",
         "Sepset-weighting effect (Sepset aggregation on both sides)"),
        ("equalW_equalSep", "equalW_noSep",
         "Sepset-aggregation effect (skeleton weight fixed = equal)"),
        ("consensusW_consensusSep", "equalW_equalSep",
         "Full proposed method vs equal-weight-but-Sepset-aware control"),
    ]
    stat_rows = []
    for a, b, desc in comparisons:
        for metric in ["F1", "Dir_F1"]:
            va = [r[a][metric] for r in rep_results]
            vb = [r[b][metric] for r in rep_results]
            stat = wilcoxon_ci(va, vb, method=f"{a} vs {b}")
            stat["description"] = desc
            stat["metric"] = metric
            stat_rows.append(stat)
    df_stat = pd.DataFrame(stat_rows)
    return df_summary, df_stat


# ======================================================================
# SECTION 2 — Consensus reliability validation 
# ======================================================================

def _client_quality(client_edges, true_skel):
    """Actual per-client skeleton F1 against ground truth."""
    q = []
    for edges in client_edges:
        _, _, _, f1 = compute_metrics(true_skel, edges)
        q.append(f1)
...
def _calibration_single(p=20, N=5000, K=5, edge_prob=0.2, hetero="mild",
                         alpha=ALPHA, ell=ELL_MAX):
    B = generate_random_dag(p, edge_prob)
    true_skel = true_skeleton_edges(B)
    X_pool = simulate_linear_sem(B, N, hetero)
    client_data = federated_split(X_pool, K)
    ...
    w_con = compute_consensus_reliability(client_edges)  # normalized, squared
    # Raw (unnormalized, unsquared) consensus score c_k, for direct comparison
    # against actual quality (Eq. 19 in the paper, before the squaring in Eq. 20).
    K_actual = len(client_edges)
    raw_c = []
    for k in range(K_actual):
        js = [len(client_edges[k] & client_edges[j]) / max(1, len(client_edges[k] | client_edges[j]))
              for j in range(K_actual) if j != k]
        raw_c.append(np.mean(js) if js else 0.0)
    raw_c = np.array(raw_c)

    quality = _client_quality(client_edges, true_skel)
    return raw_c, w_con, quality


def run_consensus_calibration(reps=REPS_DEFAULT, **kwargs):

    print(f"[2a/4] Consensus-reliability calibration ({reps} reps) ...")
    all_raw_c, all_w, all_q = [], [], []
    for _ in range(reps):
        c, w, q = _calibration_single(**kwargs)
        all_raw_c.extend(c.tolist())
        all_w.extend(w.tolist())
        all_q.extend(q.tolist())
    all_raw_c = np.array(all_raw_c)
    all_w = np.array(all_w)
    all_q = np.array(all_q)

    rho_raw, p_raw = spearmanr(all_raw_c, all_q)
    rho_w, p_w = spearmanr(all_w, all_q)

    df = pd.DataFrame({
        "raw_consensus_score": all_raw_c,
        "normalized_squared_weight": all_w,
        "actual_skeleton_F1": all_q,
    })
    summary = pd.DataFrame([{
        "spearman_rho_raw_score_vs_quality": rho_raw, "p_value_raw": p_raw,
        "spearman_rho_final_weight_vs_quality": rho_w, "p_value_weight": p_w,
        "n_client_observations": len(all_q),
    }])
    return df, summary


def _weight_regret_single(p=20, N=5000, K=5, edge_prob=0.2, hetero="mild",
                           alpha=ALPHA, tau=TAU_DEFAULT, ell=ELL_MAX):
    B = generate_random_dag(p, edge_prob)
    true_skel = true_skeleton_edges(B)
    true_dir = true_directed_edges(B)
    ...
    w_con = compute_consensus_reliability(client_edges)
    w_ora = compute_oracle_reliability(client_data, client_edges, true_skel, p)
    weight_l1_regret = float(np.sum(np.abs(w_con - w_ora)))

    def _eval(w):
        pred_skel = weighted_aggregation_with_weights(client_edges, w, tau)
        agg_seps = aggregate_sepsets(client_seps, w, tau)
        G = nx.Graph(); G.add_nodes_from(range(p)); G.add_edges_from(pred_skel)
        cpdag = orient_v_structures(G, agg_seps)
        _, _, _, f1 = compute_metrics(true_skel, pred_skel)
        _, _, _, df1 = orientation_metrics_cpdag(true_dir, cpdag)
        return f1, df1

    f1_con, df1_con = _eval(w_con)
    f1_ora, df1_ora = _eval(w_ora)

    return {
        "weight_L1_regret": weight_l1_regret,
        "F1_regret": f1_ora - f1_con,        # positive = oracle beats consensus
        "DirF1_regret": df1_ora - df1_con,
    }


def run_weight_regret(reps=REPS_DEFAULT, **kwargs):

    print(f"[2b/4] Weight/outcome regret vs oracle ({reps} reps) ...")
    rows = [_weight_regret_single(**kwargs) for _ in range(reps)]
    df = pd.DataFrame(rows)
    summary = df.agg(["mean", "std"]).T.reset_index().rename(columns={"index": "metric"})
    return df, summary


def _make_corrupted_edges(true_skel, p, corruption_rate=0.6, seed=None):
 
    rng = np.random.default_rng(seed)
    all_possible = [tuple(sorted((i, j))) for i in range(p) for j in range(i + 1, p)]
    n_target = max(1, int(len(true_skel) * (1 + corruption_rate)))
    n_target = min(n_target, len(all_possible))
    idx = rng.choice(len(all_possible), size=n_target, replace=False)
    return {all_possible[i] for i in idx}


def run_controlled_reliability_experiment(p=20, edge_prob=0.2, hetero="mild",
                                           alpha=ALPHA, tau=TAU_DEFAULT, ell=ELL_MAX,
                                           reps=REPS_DEFAULT):
 
    print(f"[2c/4] Controlled minority/corrupted/imbalanced experiment ({reps} reps) ...")
    sizes = [200, 1000, 2500, 1000, 300]
    roles = ["minority_highquality", "corrupted", "normal_large", "normal_medium", "normal_small"]

    flat_rows = []
    for rep in range(reps):
        rng_seed = SEED + 1000 + rep
        np.random.seed(rng_seed)
        random.seed(rng_seed)

        B = generate_random_dag(p, edge_prob)
        true_skel = true_skeleton_edges(B)
        client_data = [simulate_linear_sem(B, n_k, hetero) for n_k in sizes]
        client_edges, client_seps = local_pc_clients(client_data, alpha, ell)

        # Overwrite client 1's skeleton to simulate corruption; it keeps no
        # trustworthy Sepsets either.
        client_edges[1] = _make_corrupted_edges(true_skel, p, 0.6, rng_seed)
        client_seps[1] = {}

        quality = _client_quality(client_edges, true_skel)
        w_con = compute_consensus_reliability(client_edges)

        for role, n_k, q, w in zip(roles, sizes, quality, w_con):
            flat_rows.append({"rep": rep, "client_role": role, "sample_size": n_k,
                              "actual_quality": q, "consensus_weight": w})

    df = pd.DataFrame(flat_rows)
    role_summary = df.groupby("client_role")[["actual_quality", "consensus_weight"]].agg(["mean", "std"])
    return df, role_summary


def _consensus_weights_variant(client_edges, exponent=2, temperature=None, sample_sizes=None):
    ...
    for k in range(K):
        ...
        raw.append(np.mean(js) if js else 1.0 / K)
    raw = np.array(raw, dtype=float)

    if sample_sizes is not None:
        raw = raw * np.sqrt(np.array(sample_sizes, dtype=float))
    ...
    total = w.sum()
    return w / total if total > 1e-12 else np.ones(K) / K


def run_weighting_scheme_ablation(p=20, N=5000, K=5, edge_prob=0.2, hetero="mild",
                                   alpha=ALPHA, tau=TAU_DEFAULT, ell=ELL_MAX,
                                   reps=REPS_DEFAULT):
  
    print(f"[2d/4] Weighting-scheme ablation ({reps} reps) ...")
    schemes = {
        "unsquared":       dict(exponent=1, temperature=None, size_aware=False),
        "squared_default": dict(exponent=2, temperature=None, size_aware=False),
        "temperature_0.1": dict(exponent=None, temperature=0.1, size_aware=False),
        "size_aware":      dict(exponent=2, temperature=None, size_aware=True),
    }
    ...
    for rep in range(reps):
        ...
        client_data = federated_split(X_pool, K)
        client_edges, client_seps = local_pc_clients(client_data, alpha, ell)
        sample_sizes = [Xc.shape[0] for Xc in client_data]

        for name, spec in schemes.items():
            w = _consensus_weights_variant(
                client_edges,
                exponent=spec["exponent"] if spec["exponent"] is not None else 2,
                temperature=spec["temperature"],
                sample_sizes=sample_sizes if spec["size_aware"] else None,
            )
            pred_skel = weighted_aggregation_with_weights(client_edges, w, tau)
            agg_seps = aggregate_sepsets(client_seps, w, tau)
            G = nx.Graph(); G.add_nodes_from(range(p)); G.add_edges_from(pred_skel)
            ...
            shd, prec, rec, f1 = compute_metrics(true_skel, pred_skel)
            n_or, dp, dr, df1 = orientation_metrics_cpdag(true_dir, cpdag)
            rows.append({"rep": rep, "scheme": name, "SHD": shd, "F1": f1,
                        "Dir_F1": df1, "Dir_Prec": dp, "Dir_Rec": dr})

    df = pd.DataFrame(rows)
    summary = df.groupby("scheme")[["SHD", "F1", "Dir_F1"]].agg(["mean", "std"])
    return df, summary


# ======================================================================
# SECTION 3 — Mechanism-shift ground-truth estimands 
# ======================================================================

def build_target_graphs(B_base, B_client_list):

    base_skel = true_skeleton_edges(B_base)
    K = len(B_client_list)

    edge_counts = {}
    for Bc in B_client_list:
        for e in true_skeleton_edges(Bc):
            edge_counts[e] = edge_counts.get(e, 0) + 1

    union_skel = set(edge_counts.keys())
    intersection_skel = {e for e, c in edge_counts.items() if c == K}
    prevalence50_skel = {e for e, c in edge_counts.items() if c / K >= 0.5}

    return {
        "base_dag": base_skel,
        "union": union_skel,
        "intersection": intersection_skel,
        "prevalence50": prevalence50_skel,
    }


def run_mechanism_shift_multi_target(p=20, N=5000, K=5, edge_prob=0.2, hetero="mild",
                                      alpha=ALPHA, tau=TAU_DEFAULT, ell=ELL_MAX,
                                      reps=REPS_DEFAULT):
 
    print(f"[3/4] Mechanism-shift multi-target evaluation ({reps} reps) ...")
    global_rows, client_rows = [], []

    for rep in range(reps):
        B = generate_random_dag(p, edge_prob)
        B_clients = [perturb_B(B) for _ in range(K)]
        n_k = N // K
        client_data = [simulate_linear_sem(B, n_k, hetero, B_perturbed=Bc) for Bc in B_clients]

        targets = build_target_graphs(B, B_clients)
        client_edges, client_seps = local_pc_clients(client_data, alpha, ell)

        # Client-level: each client's local skeleton vs its OWN perturbed DAG.
        for k, (edges, Bc) in enumerate(zip(client_edges, B_clients)):
            own_skel = true_skeleton_edges(Bc)
            shd, prec, rec, f1 = compute_metrics(own_skel, edges)
            client_rows.append({"rep": rep, "client": k, "SHD": shd, "F1": f1,
                                "Precision": prec, "Recall": rec})

        # Global: aggregated FedPC-Consensus CPDAG vs each candidate target.
        w_con = compute_consensus_reliability(client_edges)
        pred_skel = weighted_aggregation_with_weights(client_edges, w_con, tau)
        agg_seps = aggregate_sepsets(client_seps, w_con, tau)
        Gg = nx.Graph(); Gg.add_nodes_from(range(p)); Gg.add_edges_from(pred_skel)
        # cpdag orientation not needed for skeleton-level target comparison here;
        # skeleton-level SHD/F1 already isolates the estimand-definition question.

        for target_name, target_skel in targets.items():
            shd, prec, rec, f1 = compute_metrics(target_skel, pred_skel)
            global_rows.append({"rep": rep, "target": target_name, "SHD": shd, "F1": f1,
                                "Precision": prec, "Recall": rec})

    df_global = pd.DataFrame(global_rows)
    df_client = pd.DataFrame(client_rows)
    global_summary = df_global.groupby("target")[["SHD", "F1", "Precision", "Recall"]].agg(["mean", "std"])
    client_summary = df_client[["SHD", "F1", "Precision", "Recall"]].agg(["mean", "std"])
    return df_global, df_client, global_summary, client_summary


# ======================================================================
# SECTION 4 — Sachs condition-aware loading 
# ======================================================================

OBS_EXACT_FILENAMES = ["cd3cd28.csv"]

SACHS_CANONICAL_COLS = ["praf", "pmek", "p44.42", "pakts473", "PKA", "PKC",
                         "P38", "pjnk", "plcg", "PIP2", "PIP3"]
...
def load_sachs_by_condition(sachs_dir):
    if not os.path.isdir(sachs_dir):
        return None

    frames = []
    for fn in os.listdir(sachs_dir):
        if not fn.endswith(".csv"):
            continue
        if fn.lower() == "groundtruth.csv":
            continue
        ...
        for sep in [",", "\t", ";"]:
            try:
                ...
                continue
        if df is None:
            print(f"  [WARN] Could not parse {fn}; skipping.")
            continue

        df = _rename_to_canonical(df)

        n_canon = sum(1 for c in SACHS_CANONICAL_COLS if c in df.columns)
        if n_canon < len(SACHS_CANONICAL_COLS):
            missing = [c for c in SACHS_CANONICAL_COLS if c not in df.columns]
            print(f"  [WARN] {fn}: only {n_canon}/11 canonical columns detected; "
                  f"missing={missing}")

        is_obs = fn.lower() in OBS_EXACT_FILENAMES
        df["__condition__"] = fn
        df["__is_observational__"] = is_obs
        frames.append(df)
...
def _build_ground_truth(canonical_cols):
    true_skel, true_dir = set(), set()
    for u, v in SACHS_GROUND_TRUTH_EDGES:
        if u in canonical_cols and v in canonical_cols:
            ui, vi = canonical_cols.index(u), canonical_cols.index(v)
            true_skel.add(tuple(sorted((ui, vi))))
            true_dir.add((ui, vi))
    return true_skel, true_dir


def run_sachs_by_condition(sachs_dir, K=5, tau=TAU_DEFAULT, ell=ELL_MAX, alpha=ALPHA,
                            min_samples_per_client=50, reps=10):
    combined = load_sachs_by_condition(sachs_dir)
    if combined is None:
        print("Could not load Sachs data.")
        return None

    canonical_cols = [c for c in SACHS_CANONICAL_COLS if c in combined.columns]
    if len(canonical_cols) < 5:
        print("Insufficient canonical Sachs columns found across all files "
              f"({len(canonical_cols)}/11): {canonical_cols}")
        return None

    if len(canonical_cols) < len(SACHS_CANONICAL_COLS):
        print(f"[WARN] Proceeding with a reduced variable set: "
              f"{len(canonical_cols)}/11 columns -> {canonical_cols}")

    true_skel, true_dir = _build_ground_truth(canonical_cols)
    print(f"Ground-truth restricted to detected columns: "
          f"{len(true_skel)} skeleton edges / {len(true_dir)} directed edges "
          f"(full reference = 17 edges).")

    results = []

    def eval_subset(label, sub_df, reps=reps):
        X = sub_df[canonical_cols].dropna().values.astype(float)
        n = X.shape[0]
        if n < min_samples_per_client:
            print(f"  Skipping '{label}' (n={n} too small).")
            return

        K_eff = min(K, max(2, n // min_samples_per_client))
        print(f"  {label}: n={n}, canonical_cols={len(canonical_cols)}, K_eff={K_eff}")

        rows = []
        for rep in range(reps):
            np.random.seed(SEED + rep)
            client_data = federated_split(X, K_eff)
            client_sizes = [Xc.shape[0] for Xc in client_data]
            if rep == 0 and min(client_sizes) < ell + 3:
                print(f"    [WARN] smallest client has {min(client_sizes)} samples, "
                      f"below the {ell+3} needed for ell_max={ell} CI tests; "
                      f"results for this condition may be unstable.")
            ...
            agg_seps = aggregate_sepsets(client_seps, w_con, tau)
            G = nx.Graph()
            G.add_nodes_from(range(len(canonical_cols)))
            G.add_edges_from(pred_skel)
            cpdag = orient_v_structures(G, agg_seps)
            ...
            rows.append(dict(SHD=shd, Precision=prec, Recall=rec, F1=f1,
                              n_oriented=n_or, Dir_Prec=dp, Dir_Rec=dr, Dir_F1=df1))

        dfm = pd.DataFrame(rows)
        result = {"condition": label, "n": n, "K_used": K_eff,
                  "n_canonical_cols": len(canonical_cols)}
        for col in ["SHD", "Precision", "Recall", "F1", "n_oriented", "Dir_Prec", "Dir_Rec", "Dir_F1"]:
            result[f"{col}_mean"] = dfm[col].mean()
            result[f"{col}_std"] = dfm[col].std()
        results.append(result)

    obs_df = combined[combined["__is_observational__"] == True]
    eval_subset("observational_only", obs_df)

    for cond, sub_df in combined.groupby("__condition__"):
        label = "observational" if cond.lower() in OBS_EXACT_FILENAMES else "interventional"
        eval_subset(f"{cond} [{label}]", sub_df)

    return pd.DataFrame(results)


def diagnose_pooled_loading(sachs_dir):
    if not os.path.isdir(sachs_dir):
        print(f"Sachs dir not found: {sachs_dir}")
        return None

    frames = []
    for fn in os.listdir(sachs_dir):
        if not fn.endswith(".csv") or fn.lower() == "groundtruth.csv":
            continue
        fpath = os.path.join(sachs_dir, fn)
        df = None
        for sep in [",", "\t", ";"]:
    ...
    if not frames:
        print("No Sachs files could be parsed.")
        return None

    combined = pd.concat(frames, ignore_index=True)
    available = [c for c in SACHS_CANONICAL_COLS if c in combined.columns]
    print(f"Canonical columns available after pooling all files: "
          f"{len(available)}/11 -> {available}")

    before_counts = combined["__source__"].value_counts()
    after = combined.dropna(subset=available)
    after_counts = after["__source__"].value_counts()

    report = pd.DataFrame({
        "rows_before_dropna": before_counts,
        "rows_after_dropna": after_counts.reindex(before_counts.index).fillna(0).astype(int),
    })
    report["survival_rate"] = report["rows_after_dropna"] / report["rows_before_dropna"]

    print(f"\nTotal rows before dropna: {combined.shape[0]}")
    print(f"Total rows after dropna:  {after.shape[0]}")
    print("\nPer-file survival through pooling + dropna:")
    print(report.to_string())

    if after.shape[0] > 0:
        dominant_file = after_counts.idxmax()
        dominant_frac = after_counts.max() / after.shape[0]
        print(f"\nDominant surviving file: '{dominant_file}' "
              f"({after_counts.max()}/{after.shape[0]} rows = {dominant_frac:.1%})")
        if dominant_frac > 0.95:
            print("[WARN] The 'pooled' dataset is effectively a single file after "
                  "dropna. Section 4.2.3's description of pooling all experimental "
                  "conditions likely needs correction.")
...
if __name__ == "__main__":
    ...
    # ---- 1. Factorial ablation ----
    df_fact, df_fact_stat = run_factorial_ablation(reps=REPS_DEFAULT)
    df_fact.to_csv(f"{OUT_DIR}/factorial_ablation_summary.csv", index=False)
    df_fact_stat.to_csv(f"{OUT_DIR}/factorial_ablation_wilcoxon.csv", index=False)

    # ---- 2. Consensus reliability validation ----
    df_calib, calib_summary = run_consensus_calibration(reps=REPS_DEFAULT)
    df_calib.to_csv(f"{OUT_DIR}/consensus_calibration_raw.csv", index=False)
    calib_summary.to_csv(f"{OUT_DIR}/consensus_calibration_summary.csv", index=False)

    df_regret, regret_summary = run_weight_regret(reps=REPS_DEFAULT)
    df_regret.to_csv(f"{OUT_DIR}/weight_regret_raw.csv", index=False)
    regret_summary.to_csv(f"{OUT_DIR}/weight_regret_summary.csv", index=False)

    df_controlled, controlled_summary = run_controlled_reliability_experiment(reps=REPS_DEFAULT)
    df_controlled.to_csv(f"{OUT_DIR}/controlled_reliability_raw.csv", index=False)
    controlled_summary.to_csv(f"{OUT_DIR}/controlled_reliability_summary.csv")

    df_weightscheme, weightscheme_summary = run_weighting_scheme_ablation(reps=REPS_DEFAULT)
    df_weightscheme.to_csv(f"{OUT_DIR}/weighting_scheme_ablation_raw.csv", index=False)
    weightscheme_summary.to_csv(f"{OUT_DIR}/weighting_scheme_ablation_summary.csv")

    # ---- 3. Mechanism-shift multi-target ----
    df_g, df_c, g_summary, c_summary = run_mechanism_shift_multi_target(reps=REPS_DEFAULT)
    df_g.to_csv(f"{OUT_DIR}/mechshift_global_raw.csv", index=False)
    df_c.to_csv(f"{OUT_DIR}/mechshift_client_raw.csv", index=False)
    g_summary.to_csv(f"{OUT_DIR}/mechshift_global_summary.csv")
    c_summary.to_csv(f"{OUT_DIR}/mechshift_client_summary.csv")
    ...
    if not os.path.isdir(SACHS_DIR):
        ...
        print("=== Diagnosing pooled-loading behavior (main experiment) ===")
        diag_report = diagnose_pooled_loading(SACHS_DIR)
        if diag_report is not None:
            diag_report.to_csv(f"{OUT_DIR}/sachs_pooling_diagnostic.csv")

        print("\n=== Per-condition evaluation (fixed column mapping, 10 reps) ===")
        df_result = run_sachs_by_condition(SACHS_DIR, reps=10)
        if df_result is not None:
            out_path = f"{OUT_DIR}/sachs_by_condition.csv"
